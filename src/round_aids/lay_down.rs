...
pub fn calculate_score(hand: &Vec<Card>) -> u32 { 
    let mut value_groups: HashMap<Value, Vec<&Card>> = HashMap::new(); let mut suit_groups: HashMap<Suit, Vec<&Card>> = HashMap::new(); let mut wilds = 0; for card in hand { if card.suit == Suit::Wild { wilds += 1; } else { suit_groups.entry(card.suit).or_default().push(card); } value_groups.entry(card.value).or_default().push(card); } 
